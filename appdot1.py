...
def download_pdf(csv_filename):
    ...
    if buyer_sentence is None or invoice_number is None:
        return render_template('upload.html', message="Buyer sentence or invoice number not found for the given CSV file."), 400

    # Generate PDF if it doesn't exist
    if not os.path.exists(output_pdf_path):
        csv_to_pdf(csv_path, output_pdf_path, buyer_sentence, invoice_number)

    # Serve the PDF file for download with no caching
    return send_from_directory(app.config['PDF_OUTPUT_FOLDER'], pdf_filename, as_attachment=True, max_age=0)

# Other functions like csv_to_pdf(), csv_to_pdf1(), etc...
