...
def download_pdf1(csv_filename,buyer_sentences=None):
    # Paths
    csv_path = os.path.join(app.config['UPLOAD_FOLDER'], csv_filename)
    ...
    buyer_sentences = request.args.getlist('buyer_sentences')
    # Generate PDF
    csv_to_pdf1(csv_path, output_pdf_path1,buyer_sentences)
...
def csv_to_pdf1(csv_path, output_pdf_path1,buyer_sentences=None):
    ...
    if buyer_sentences:
        for sentence in buyer_sentences:
            pdf.multi_cell(17, 55, sentence, align='L')
...
def csv_to_pdf(csv_path, output_pdf_path):
    ...
    # Set margin and frame dimensions
    margin = 5
    frame_width = pdf.w - 2 * margin
    frame_height = pdf.h - 2 * margin

    # Draw the frame
    pdf.rect(margin, margin, frame_width, frame_height)
     # Define column widths
    index_col_width = 25
    combined_col_width = 180
    third_col_width = 35
    sub_col_width = 20

    # Calculate total table width
    total_table_width = index_col_width + combined_col_width + third_col_width + sub_col_width

    # Get PDF page width (excluding margins)
    page_width = pdf.w - 0.5 * pdf.l_margin

    # Calculate starting X position to center the table
    start_x = (page_width - total_table_width) / 2

    # Adding table header with custom widths
    pdf.set_font('Arial', '', 8)

    # Set the X position to the calculated start position
    pdf.set_x(start_x)

    # Header for the first column
    pdf.cell(index_col_width, 10, 'No', 1)  # Width = 10

    # Header for the combined columns
    pdf.add_font('DejaVu', '', 'DejaVuSansCondensed.ttf', uni=True)
    pdf.set_font("DejaVu", '', size=8)
    pdf.cell(combined_col_width, 10, 'Товар', 1)  # Width = 50

    # Header for the third column and its sub-column
    pdf.cell(third_col_width, 10, 'Количество', 1)  # Width = 70
    pdf.cell(sub_col_width, 10, 'Ед', 1)          # Width = 20

    pdf.ln()

    # Adding table rows with custom cell widths
    pdf.set_font('Arial', '', 10)
    for i, (index, row) in enumerate(data.iterrows(), start=1):
        # Set the X position to the calculated start position for each row
        pdf.set_x(start_x)
        
        # Print the custom index first (starting from 1)
        pdf.cell(index_col_width, 7, str(i), 1)  # Width = 10
        
        # Combined cell for the first two columns
        combined_text = f"{row[0]} {row[1]}"  # Adjust as necessary
        pdf.add_font('DejaVu', '', 'DejaVuSansCondensed.ttf', uni=True)
        pdf.set_font("DejaVu", '', size=8)
        pdf.cell(combined_col_width, 7, combined_text, 1)  # Width = 50

        # Data for the third column and "шт"
        pdf.cell(third_col_width, 7, str(row[2]), 1)  # Width = 70
        pdf.cell(sub_col_width, 7, 'шт', 1)           # Width = 20

        pdf.ln()

    # Adjust Y position after the table
    pdf.set_y(pdf.get_y() + 1.04)
